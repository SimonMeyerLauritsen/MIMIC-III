CREATE PROCEDURE [dbo].[InsertSuspinfect_poe_day1]
AS

-- only works for metavision as carevue does not accurately document antibiotics
DROP TABLE IF EXISTS suspinfect_poe_day1;
with mv as
(
  select hadm_id
...
(
  ...
  left join mv
      ...
      between DATEADD(DAY, -1, ie.intime) and ie.outtime
)
, me as
(
  select hadm_id
...
(
  ...
  left join me me72
    ...
    (
      ...
      and ab_tbl.first_antibiotic_time <= DATEADD(HOUR, 72, me72.charttime)
      )
      OR
      (
      ...
      and ab_tbl.first_antibiotic_time < DATEADD(HOUR, 96, me72.chartdate) -- could equally do this with a date_trunc, but that's less portable
      )
    )
  ...
  left join me me24
    ...
    (
      -- if charttime is available, use it
      (
          ab_tbl.first_antibiotic_time > DATEADD(HOUR, -24, me24.charttime)
      ...
      and ab_tbl.first_antibiotic_time <= DATEADD(HOUR, 24, me24.chartdate)
      )
    )
)
, ab_laststg as
...
select
  ...
  , case
      when first_antibiotic_time > DATEADD(HOUR, 48, intime) then null
...
select
  ...
  , DATEADD(HOUR, -48, suspected_infection_time) as si_starttime
  , DATEADD(HOUR, 24, suspected_infection_time) as si_endtime
  , specimen, positiveculture
  , first_antibiotic_name
  , first_antibiotic_time
INTO dbo.suspinfect_poe_day1
from ab_laststg
order by icustay_id;
GO
