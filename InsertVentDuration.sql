CREATE PROCEDURE [dbo].[InsertVentDuration]
AS
  ...
  select
      ...
      , case
        -- if there is an extubation flag, we mark any subsequent ventilation as a new ventilation event
          ...
          when DATEDIFF(HOUR, charttime_lag, CHARTTIME) > 8
            then 1
        else 0
        end as newvent
...
select icustay_id
  ...
  , DATEDIFF(HOUR, min(charttime), max(charttime)) AS duration_hours

into dbo.VENTDURATIONS
from vd2
