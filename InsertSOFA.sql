CREATE PROCEDURE [dbo].[InsertSOFA]
AS
...
(
  ...
  AND valuenum != 0
  and charttime between DATEADD(DAY, -1, ie.intime) AND DATEADD(DAY, +1, ie.intime)
...
/*, echo2 as(
  ...
  left join echodata echo
    ...
    and echo.charttime > DATEADD(DAY, -7, ie.intime)
    and echo.charttime < DATEADD(DAY, 1, ie.intime)
  group by ie.icustay_id*/
--)
, vaso_cv as
(
  ...
  inner join inputevents_cv cv
    on ie.icustay_id = cv.icustay_id and cv.charttime between ie.intime and DATEADD(DAY, 1, ie.intime)
  left join wt
...
(
  ...
  inner join inputevents_mv mv
    on ie.icustay_id = mv.icustay_id and mv.starttime between ie.intime and DATEADD(DAY, 1, ie.intime)
...
/****** Object:  StoredProcedure [dbo].[InsertSuspectInf]    Script Date: 23-11-2020 11:50:09 ******/
SET ANSI_NULLS ON
GO
