...
select ie.icustay_id, ie.hadm_id
    ...
    , round((DATEDIFF(DAY, cast(pat.dob as date), cast(adm.admittime as date)) / 365.242), 4) as age
    , pat.gender
    , adm.ethnicity
    , ie.dbsource
    ...
    , DATEDIFF(DAY, ie.intime, spoe.suspected_infection_time) as suspected_infection_time_poe_days
...
select
  ...
  , case when t1.suspected_infection_time_poe is not null
          and t1.suspected_infection_time_poe < DATEADD(DAY, 1, t1.intime) then 1
  ...
  , case when t1.suspected_infection_time_poe is not null  -- CHANGED FROM ORIGINAL!
          and t1.suspected_infection_time_poe > DATEADD(HOUR, 4, t1.intime) then 1
  ...
  , case when
          ...
          or (
              ...
              and t1.suspected_infection_time_poe < DATEADD(HOUR, 4, t1.intime)
            )
