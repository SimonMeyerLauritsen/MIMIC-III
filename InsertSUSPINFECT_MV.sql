CREATE PROCEDURE [dbo].[InsertSUSPINFECT_MV]
AS
/*
- ORIGINAL VERSION: https://github.com/alistairewj/sepsis3-mimic/blob/master/query/tbls/abx-poe-list.sql
- DOWNLOADED FROM (MODIFIED VERSION): https://github.com/BorgwardtLab/mgp-tcn/tree/master/src/query
*/

DROP TABLE IF EXISTS dbo.SUSPINFECT_MV;
with t1 as
(
  select
    drug, 
  ...
  and route not like '%ear%'
  and route not like '%eye%'
  -- we exclude certain types of antibiotics: topical creams, gels, desens, etc
  and drug not like '%cream%'
  and drug not like '%desensitization%'
  and drug not like '%ophth oint%'
  and drug not like '%gel%'
)
select
  drug --, drug_name_generic
  , count(*) as numobs
into dbo.SUSPINFECT_MV
from t1
where antibiotic = 1
