CREATE PROCEDURE [dbo].[InsertSuspinfect_poe_iv]
AS
/*
- ORIGINAL VERSION: https://github.com/alistairewj/sepsis3-mimic/blob/master/query/tbls/abx-micro-prescription.sql
- DOWNLOADED FROM (MODIFIED VERSION): https://github.com/BorgwardtLab/mgp-tcn/tree/master/src/query
*/

-- Checked for DATEADD

-- only works for metavision as carevue does not accurately document antibiotics
DROP TABLE IF EXISTS dbo.suspinfect_poe_iv;
with mv as
(
  select hadm_id
...
(
  ...
  left join me me72
    ...
    (
      ...
      and ab_tbl.antibiotic_time <= DATEADD(HOUR, 72, me72.charttime)
      )
      OR
      (
      ...
      and ab_tbl.antibiotic_time < DATEADD(HOUR, 96, me72.chartdate)-- could equally do this with a date_trunc, but that's less portable
      )
    )
  ...
  left join me me24
    ...
    (
      -- if charttime is available, use it
      (
          ab_tbl.antibiotic_time > DATEADD(HOUR, -24, me24.charttime)
      ...
      and ab_tbl.antibiotic_time <= DATEADD(HOUR, 24, me24.chartdate)
      )
    )
)
, ab_laststg as
...
select
  ...
  , DATEADD(HOUR, -48, suspected_infection_time) as si_starttime --CHANGE FROM ORIGINAL: uncommented this and next line such that start and end time are calculated!
  , DATEADD(HOUR, 24, suspected_infection_time) as si_endtime
  , specimen, positiveculture
into dbo.suspinfect_poe_iv
from ab_laststg
